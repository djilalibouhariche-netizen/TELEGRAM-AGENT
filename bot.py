...
environment are used â€” you can enable just the free ones (Gemini + Groq) and
skip the paid ones entirely.

Uses litellm (https://github.com/BerriAI/litellm) so every provider is called
through the same unified, OpenAI-style interface â€” including tool calling.
...
SYSTEM_PROMPT = (
    ...
    "to you in the conversation â€” use read_pdf or read_excel with that path. "
...
# ---- are actually used â€” you don't need to fill all of them in, just set
...
def _call_model_with_fallback(messages: list):
    ...
    for provider in PROVIDERS:
        try:
            ...
            logger.warning("Provider '%s' failed (%s) â€” trying next provider.", provider["name"], exc)
            last_error = exc
    raise last_error
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Ø£Ù‡Ù„Ø§Ù‹! Ø£Ù†Ø§ Ù…Ø³Ø§Ø¹Ø¯Ùƒ Ø§Ù„Ø°ÙƒÙŠ. Ø£Ø±Ø³Ù„ Ù„ÙŠ Ø£ÙŠ Ø±Ø³Ø§Ù„Ø©ØŒ Ø£Ùˆ Ù…Ù„Ù PDF/ExcelØŒ ÙˆØ³Ø£Ø³Ø§Ø¹Ø¯Ùƒ.\n"
        "Ø£Ø±Ø³Ù„ /reset Ù„Ù…Ø³Ø­ Ø°Ø§ÙƒØ±Ø© Ø§Ù„Ù…Ø­Ø§Ø¯Ø«Ø©."
    )


async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    conversations.pop(update.effective_chat.id, None)
    await update.message.reply_text("ØªÙ… Ù…Ø³Ø­ Ø°Ø§ÙƒØ±Ø© Ø§Ù„Ù…Ø­Ø§Ø¯Ø«Ø©. Ø§Ø¨Ø¯Ø£ Ù…Ù† Ø¬Ø¯ÙŠØ¯!")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        reply, attachments = "Ø¹Ø°Ø±Ù‹Ø§ØŒ Ø­Ø¯Ø« Ø®Ø·Ø£ Ø£Ø«Ù†Ø§Ø¡ Ù…Ø¹Ø§Ù„Ø¬Ø© Ø·Ù„Ø¨Ùƒ. Ø­Ø§ÙˆÙ„ Ù…Ø±Ø© Ø£Ø®Ø±Ù‰.", []
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        reply, attachments = "Ø¹Ø°Ø±Ù‹Ø§ØŒ Ø­Ø¯Ø« Ø®Ø·Ø£ Ø£Ø«Ù†Ø§Ø¡ Ù…Ø¹Ø§Ù„Ø¬Ø© Ø§Ù„Ù…Ù„Ù.", []
